...
function recipeCard(recipe) {
	...
	card.querySelector('.recipe-image').src = recipe.image
	card.querySelector('.recipe-image').alt = recipe.name
	card.querySelector('.tag').textContent = (recipe.tags[0] || 'recipe').toLowerCase()
	card.querySelector('h2').textContent = recipe.name
	card.querySelector('.description').textContent = recipe.description

	const rating = card.querySelector('.rating')
	rating.textContent = stars(recipe.rating)
...
function showRecipes(list) {
	...
	list.forEach((recipe) => container.append(recipeCard(recipe)))
}
